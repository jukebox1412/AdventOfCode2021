...
def main():
    ...
    print(count_flashes(octopus_map, 100))
        ...
        for row in range(0, len(octopus_map)):
            for col in range(0, len(octopus_map[0])):
                if octopus_map[row][col] == 10:
                    to_flash_list.append((row, col))
                    octopus_map[row][col] += 1 # add one to it so we don't double flash

        for to_flash in to_flash_list:
            row = to_flash[0]
            col = to_flash[1]
            ...
            for dir in dirs:
                if dir and octopus_map[dir[0]][dir[1]] < 10: # less than 10 because we're checking for 10 exactly
